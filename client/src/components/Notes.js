...
function NotesPage() {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      const updatedNotes = notes.map((n) => (n._id === editNote._id ? res.data : n));
      setNotes(updatedNotes);
      setFilteredNotes(updatedNotes);
  ...
  const handleSort = (type) => {
    let sorted = [...notes];
    if (type === "recent") sorted.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
    else if (type === "oldest") sorted.sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt));
    else if (type === "range") {
      sorted = notes.filter(
        (n) =>
          (!dateRange.from || new Date(n.createdAt) >= new Date(dateRange.from)) &&
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 2, mb: 2 }}>
      ...
      {/* Notes List */}
      <Paper elevation={4} sx={{ height: { xs: 400, sm: 500 }, display: "flex", flexDirection: "column", borderRadius: 3, overflowY: "auto", p: 1 }}>
              ...
              >
                <Box sx={{ flex: 1 }}>
                  <ListItemText
                    ...
                    secondary={new Date(note.createdAt).toLocaleString(undefined, {
                      dateStyle: "medium",
                      timeStyle: "short",
                    })}
                  />
                ...
                <Box sx={{ display: "flex", gap: 1 }}>
                  ...
                  <IconButton color="error" onClick={() => { setMenuNoteId(note._id); setDeleteDialogOpen(true); }}>
      ...
      <Dialog open={addDialogOpen} onClose={() => setAddDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
          {/* Toolbar */}
          <Box sx={{ display: "flex", gap: 1 }}>
            <Button size="small" onClick={() => execCommand("bold", null, false)}><b>B</b></Button>
            <Button size="small" onClick={() => execCommand("underline", null, false)}><u>U</u></Button>
            <Button size="small" onClick={() => handleAddLink(false)}>Link</Button>
            <Button size="small" onClick={() => handleCopy(false)}>Copy</Button>
          </Box>
          ...
          <Box
            ...
            ref={addEditorRef}
            sx={{ border: "1px solid #ccc", borderRadius: 1, minHeight: 100, p: 1, overflowY: "auto" }}
          />

          <Button variant="outlined" component="label">
            Upload Image
            <input type="file" hidden accept="image/*" onChange={(e) => e.target.files[0] && setNewImage(e.target.files[0])} />
          </Button>
          {newImage && (
            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
              <Typography variant="body2">{newImage.name}</Typography>
              <IconButton size="small" onClick={() => setNewImage(null)}><CloseIcon fontSize="small" /></IconButton>
            </Box>
          )}
        </DialogContent>
        <DialogActions sx={{ justifyContent: "center", gap: 2 }}>
          <Button sx={{ bgcolor: "#bb2124", color: "#fff" }} onClick={() => setAddDialogOpen(false)}>Cancel</Button>
          <Button variant="contained" sx={{ bgcolor: "#0077B5" }} onClick={handleAddNote}>Add</Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={editDialogOpen} onClose={() => setEditDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
          {/* Toolbar */}
          <Box sx={{ display: "flex", gap: 1 }}>
            <Button size="small" onClick={() => execCommand("bold")}><b>B</b></Button>
            <Button size="small" onClick={() => execCommand("underline")}><u>U</u></Button>
            <Button size="small" onClick={() => handleAddLink()}>Link</Button>
          ...
          <Button variant="outlined" component="label">
            Upload New Image
            <input type="file" hidden accept="image/*" onChange={(e) => e.target.files[0] && setEditImage(e.target.files[0])} />
          </Button>
          {editImage && (
            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
              <Typography variant="body2">{editImage.name}</Typography>
              <IconButton size="small" onClick={() => setEditImage(null)}><CloseIcon fontSize="small" /></IconButton>
            </Box>
          )}
        </DialogContent>
        <DialogActions sx={{ justifyContent: "center", gap: 2 }}>
          <Button onClick={() => setEditDialogOpen(false)}>Cancel</Button>
          <Button variant="contained" sx={{ bgcolor: "#0077B5" }} onClick={handleSaveEdit}>Save</Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={imageDialogOpen} onClose={() => setImageDialogOpen(false)} maxWidth="md">
        <DialogContent sx={{ position: "relative", p: 0, display: "flex", justifyContent: "center", alignItems: "center", backgroundColor: "transparent" }}>
          <IconButton
            onClick={() => setImageDialogOpen(false)}
            sx={{ position: "absolute", top: 8, right: 8, color: "white", backgroundColor: "rgba(0,0,0,0.4)", "&:hover": { backgroundColor: "rgba(0,0,0,0.6)" } }}
          >
            <CloseIcon />
          </IconButton>
          <Box component="img" src={imageDialogSrc} alt="enlarged" sx={{ maxWidth: "100%", maxHeight: "90vh", objectFit: "contain", borderRadius: 1 }} />
        </DialogContent>
      </Dialog>
